// import { CircularProgress } from '@material-ui/core';
import React, { useEffect, useState } from 'react';
import { Button } from '@material-ui/core';

// import { connect } from 'react-redux';
// import { setCurrentChain, setCurrentSignerAddress, setWeb3Buffering } from '../../../../service/redux/actions';
import { abbreviateAddress, getChainId, initWeb3 } from './utils';

function Web3GlobalProvider() {

    // const { web3state, setCurrentChain, setCurrentSignerAddress, setWeb3Buffering } = props;
    const [currentSignerAddress, setCurrentSignerAddress] = useState('');
    const [currentChain, setCurrentChain] = useState('');
    const [web3Buffering, setWeb3Buffering] = useState(false)
    useEffect(async () => {
        if (typeof window !== "undefined") {
            if (typeof window.ethereum !== 'undefined' || typeof window.web3 !== 'undefined') {
                // 1. get metamask signer from ether.js
                // 2. get current chain id
                // 3. check user metamask connection -> set currentSignerAddress

                console.log('window.ethereum', window.parent.ethereum, window.parent.web3)//$readable.hide()

                const signer = await initWeb3();
                if (signer) {
                    // set user wallet address
                    setCurrentSignerAddress(await signer.getAddress())
                }

                // if (web3state.currentChain.chainId === -1) {
                //     // set chain 
                //     updateNetwork();
                // };

                console.log({ currentChain })
                window.ethereum.on("chainChanged", async (_chainId) => {
                    console.log('listening to chainChanged event');
                    setWeb3Buffering(true);
                    updateNetwork();
                })
                window.ethereum.on('accountsChanged', (acc) => {
                    console.log('listening to accountsChanged event')
                    setWeb3Buffering(true);
                    updateWalletAddress(acc);
                })
            } else {

                console.log('else', window.chrome.windows, window.chrome)

                console.log('metamask is not installed');
                // setIsError({status: true, message: 'metmask-not-installed'})
            }
        }

        return () => {
            window.ethereum.removeAllListeners();
        }
    }, []);

    const updateNetwork = async () => {
        const _chain = await getChainId();
        setCurrentChain(_chain);
        setWeb3Buffering(false);
    };

    const updateWalletAddress = async (acc) => {
        if (acc.length > 0) {
            setCurrentSignerAddress(acc[0]);
            updateNetwork();
        } else {
            setCurrentSignerAddress("");
            setWeb3Buffering(false);
        }
    };
    const customNetworkChainName = (networkName) => {
        let networkText = String(networkName);
        switch (networkName) {
            case 'homestead':
                networkText = 'ETHEREUM'
                break
            default:
                networkText = networkText
        }
        return String(networkText).toUpperCase()
    };

    const handleMetamaskConnect = async () => {
        console.log('handleMetamaskConnect', window, 'windowParent', window.parent.window, window.ethereum)
        // if (chrome.window) {
        //     chrome.window.ethereum.request({ method: 'eth_requestAccounts' })
        // }
        window.chrome.windows.ethereum.request({ method: 'eth_requestAccounts' })
        // window.ethereum.request({ method: 'eth_requestAccounts' })
    };

    /* <span>Current Address: {currentSignerAddress}</span> */
    // <span>Current Chain: {currentChain}</span>
    return <React.Fragment>
        <div>
            {/* <a href="https://chrome.google.com/webstore/detail/metamask/nkbihfbeogaeaoehlefnkodbefgpgknn"
            target="_blank"
            rel="noreferrer noopener"
            >Install Metamask
            </a> */}

            {/* iFrame to access chrome's browser */}

            {currentSignerAddress ? (
                <div className="network-button-group order-summary-button">
                    <Button
                        className="network-button"
                        color={"primary"}
                        variant={"contained"}
                        disabled={true}
                    >
                        <span>{`${customNetworkChainName(currentChain['name'])}: ${abbreviateAddress(currentSignerAddress)}`}</span>
                        <img className="nav-metamask-icon order-summary-metamask-icon"
                            src="data:image/png;base64,"
                            alt=""
                        />
                        <span className="network-button-status">●</span>
                    </Button>
                </div>
            ) : (
                <Button
                    className="connect-metamask-button"
                    color="primary"
                    variant="outlined"
                    onClick={handleMetamaskConnect}
                >
                    Connect Metamask
                </Button>
            )}
        </div>
    </React.Fragment>
}

export default Web3GlobalProvider;
